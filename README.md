...
**5분마다 자동 갱신**되며, 네트워크 오류 시 마지막 캐시 데이터를 표시합니다. Dock 우측의 **↻ 새로고침 버튼**으로 즉시 갱신할 수 있습니다 (1분 쿨타임).

### 미리보기

<img width="440" height="89" alt="image" src="https://github.com/user-attachments/assets/32c7d6fb-b854-4c3f-a04b-a43276c29e7e" />

```
⚡  Session 100%        📅  Weekly 2%
   resets in now            resets in 6d 20h
```
...
이 확장 프로그램은 **자가 서명(self-signed) 인증서**로 패키징되어 있습니다.

- Windows가 설치 시 **보안 경고창을 띄우는 것은 정상**입니다.
- `install.ps1`이 **인증서 신뢰 등록을 자동으로 처리**합니다. 별도로 인증서를 가져올 필요 없습니다.
- 소스 코드가 [공개](https://github.com/iceberggymnast/powertoys-claude-usage)되어 있으므로 **직접 검증 가능**합니다.

### 설치 방법

#### Option A — 간편 설치 (권장)

1. [Releases](https://github.com/iceberggymnast/powertoys-claude-usage/releases) 페이지에서 `DockBar.msix`, `DockBar.cer`, `install.ps1` 다운로드
2. `install.ps1` 우클릭 → **관리자 권한으로 실행**
3. PowerToys 재시작
...
1. **저장소 클론**
   ```powershell
   git clone https://github.com/iceberggymnast/powertoys-claude-usage.git
   cd powertoys-claude-usage
   ```
...
GET https://api.anthropic.com/api/oauth/usage
  ...
  seven_day.utilization / resets_at
        │
        ▼
PowerToys Dock Band (5분 자동 갱신 / ↻ 버튼으로 즉시 갱신)
```
...
**Auto-refreshes every 5 minutes.** Shows cached data on network errors. A **↻ refresh button** on the Dock allows instant refresh with a 1-minute cooldown.

### Preview

<img width="440" height="89" alt="image" src="https://github.com/user-attachments/assets/4cbfd5cb-7270-4082-8a94-7dea3781750c" />
...
This extension is packaged with a **self-signed certificate**.

- Windows will show a **security warning during installation — this is expected**.
- `install.ps1` **handles certificate trust registration automatically**. No manual certificate import needed.
- The source code is [public](https://github.com/iceberggymnast/powertoys-claude-usage) — **feel free to audit it yourself**.

### Installation

#### Option A — Easy Install (Recommended)

1. Go to [Releases](https://github.com/iceberggymnast/powertoys-claude-usage/releases) and download `DockBar.msix`, `DockBar.cer`, and `install.ps1`
...
The extension reads the Claude Code OAuth token from `~/.claude/.credentials.json` and calls the Claude usage API every 5 minutes. Results are displayed as a Dock band in the PowerToys Command Palette toolbar.

A **↻ refresh button** allows instant manual refresh with a 1-minute cooldown. On HTTP 429 or transient errors, the last successful response is shown with the error code as subtitle.
